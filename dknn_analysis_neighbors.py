...
amount_points = 30000
backend = NearestNeighbor.BACKEND.FAISS #TODO FALCONN does not work for small amount of data, most likely because data is in different buckets (LSH) --> if wanted, an alternative has to be implemented
...
generated_neighbors_per_scale_member_all = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, (max_amount_generate_neighbors+amount_calibration), 28, 28, 1))
generated_neighbors_per_scale_non_member_all = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, (max_amount_generate_neighbors+amount_calibration), 28, 28, 1))

# Use a holdout of the generated neighbors to simulate calibration data for the DkNN.
calibration_data_per_scale_member= np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, amount_calibration, 28, 28, 1))
calibration_data_per_scale_non_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, amount_calibration, 28, 28, 1))

calibration_label_per_scale_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, amount_calibration), dtype= np.uint8)
calibration_label_per_scale_non_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, amount_calibration), dtype= np.uint8)

generated_neighbors_per_scale_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, max_amount_generate_neighbors, 28, 28, 1))
generated_neighbors_per_scale_non_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, max_amount_generate_neighbors, 28, 28, 1))

label_generated_neighbors_per_scale_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, max_amount_generate_neighbors), dtype= np.uint8)
label_generated_neighbors_per_scale_non_member = np.zeros(shape=(len(SCALES), AMOUNT_M_NM_TOTAL, max_amount_generate_neighbors), dtype= np.uint8)
...
for scale_ind in range(len(SCALES)):
    for element in range(AMOUNT_M_NM_TOTAL):
        generated_neighbors_per_scale_member_all[scale_ind][element] = generate_neighboring_points(member_data[element], amount=(max_amount_generate_neighbors+amount_calibration), scale= SCALES[scale_ind])
        calibration_data_per_scale_member[scale_ind][element] = generated_neighbors_per_scale_member_all[scale_ind][element][:amount_calibration]
        generated_neighbors_per_scale_member[scale_ind][element] = generated_neighbors_per_scale_member_all[scale_ind][element][amount_calibration:]

        label_generated_neighbors_per_scale_member[scale_ind][element] = np.full((max_amount_generate_neighbors),member_labels[element])
        label_generated_neighbors_per_scale_member[scale_ind][element] = np.full((max_amount_generate_neighbors), member_labels[element])

        generated_neighbors_per_scale_non_member_all[scale_ind][element] = generate_neighboring_points(non_member_data[element], amount=(max_amount_generate_neighbors+amount_calibration), scale=SCALES[scale_ind])
        calibration_data_per_scale_non_member[scale_ind][element] = generated_neighbors_per_scale_non_member_all[scale_ind][element][:amount_calibration]
        generated_neighbors_per_scale_member[scale_ind][element] = generated_neighbors_per_scale_non_member_all[scale_ind][element][amount_calibration:]

        calibration_label_per_scale_non_member[scale_ind][element] =np.full((amount_calibration), non_member_labels[element])
        label_generated_neighbors_per_scale_non_member[scale_ind][element] = np.full((max_amount_generate_neighbors), non_member_labels[element])
...
for scale, k_neighbors, amount_generate_neighbors in HYPERPARAMETERS:
    ...
    for element in range(AMOUNT_M_NM_TOTAL):
        ...
        dknn_member = DkNNModel(
            ...
            train_data=generated_neighbors_per_scale_member[scale_index][element][:amount_generate_neighbors],
            train_labels=label_generated_neighbors_per_scale_member[scale_ind][element][:amount_generate_neighbors],
            back_end= backend
        )
        ...
        dknn_non_member = DkNNModel(
            ...
            layers=nb_layers,
            get_activations=get_activations,
            train_data=generated_neighbors_per_scale_non_member[scale_index][element][:amount_generate_neighbors],
            train_labels=label_generated_neighbors_per_scale_non_member[scale_ind][element][:amount_generate_neighbors],
            back_end=backend,
        )

        cal_data_temp = calibration_data_per_scale_member[scale_index][element][:amount_calibration]
        ...
        dknn_member.calibrate(calibration_data_per_scale_member[scale_index][element],
                              calibration_label_per_scale_member[scale_index][element])

        dknn_non_member.calibrate(calibration_data_per_scale_non_member[scale_index][element][:amount_calibration],
                              calibration_label_per_scale_non_member[scale_index][element][:amount_calibration])


        # forward propagation through DkNNs
        _, knns_ind_member, _, knns_distances_member = dknn_member.fprop_np(member_data[element], get_knns=True)
        _, knns_ind_non_member, _, knns_distances_non_member = dknn_non_member.fprop_np(non_member_data[element], get_knns=True)
