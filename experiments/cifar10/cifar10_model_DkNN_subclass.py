...
def get_activations(data) -> dict:
    ...
    )  # TODO the problem is that the input is not retrieved but when it is added manually (like in build_grapH(), the outputs are unconnected to it which throws the error
