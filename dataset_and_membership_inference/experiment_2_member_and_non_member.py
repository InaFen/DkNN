    ...
    for iteration in range(amount_iterations):
        ...
        for percentage_members in amount_members:
            p_values_iteration.append(
                experiment[f"iteration: {iteration}"][
                    f"percentage members: {percentage_members}"
                ]["p value"]
            )
        plot_p_value_exp2(
            amount_members=amount_members,
            p_values=p_values_iteration,
            title=title.format(iteration),
        )
...
def plot_mean_all_iterations_exp2(experiment, title: str) -> None:
    ...
    for iteration in range(amount_iterations):
        ...
        for percentage_members_index in range(len(amount_members)):
            p_values[percentage_members_index].append(
                p_values_iteration[percentage_members_index]
            )
    ...
    plot_p_value_exp2(
        amount_members=amount_members, p_values=p_values_mean, title=title
    )


plot_iteration_exp2(
    experiment_2_members, title="Exp. 2, iteration {}"
)  #: Random non-members, percentage of random members and non-members as <<members>>")
plot_mean_all_iterations_exp2(
    experiment_2_members, title="Exp. 2, mean over 10 iterations"
)  #: Random non-members, percentage of random members and non-members as <<members>>")
