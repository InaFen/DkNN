...
def print_inference(outputs_train, outputs_test) -> None:
    ...
    mean_prediction_test, mean_prediction_train = (
        outputs_test[:, 0].mean(),
        outputs_train[:, 0].mean(),
    )
    # get p value (calculated through T-test)
    p_value = get_p(outputs_train, outputs_test)
    print(
        f"p-value = {p_value} \t| Mean difference of regressor prediction btw. member and non-member data = {mean_prediction_test-mean_prediction_train}"
    )


# get part of the output
outputs_regressor_train_partly, outputs_regressor_test_partly = (
    outputs_regressor_train[split_index:],
    outputs_regressor_test[split_index:],
)
...
if not os.path.exists(
    ...
    for repition_number in r_pbar:
        ...
        for amount_examples_at_once in m_list:
            ...
            p_values = get_p_values(
                amount_examples_at_once,
                outputs_regressor_train_partly,
                outputs_regressor_test_partly,
                repitions_get_p,
            )
            try:
            ...
            pbar.set_description(
                f"{repition_number: 2d} amount_examples_at_once={amount_examples_at_once: 3d}: f{harmonic_mean: 0.5f}"
            )
    ...
    for name, rep_dict in p_values_all_threat_models_dict.items():
        ...
        df_list.append(
            pd.melt(
                df,
                id_vars=["amount_examples_at_once"],
                var_name="repition_number",
                value_name="p_value",
            ).assign(threat_model=name)
        )
...
plt.clf()
ax = sns.lineplot(
    x="amount_examples_at_once", y="p_value", data=results_df_no_duplicates
)
