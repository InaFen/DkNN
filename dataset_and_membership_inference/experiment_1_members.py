...
amount_data_from_dataset = max(data_amounts)*amount_iterations #how much data should be used to get distances
...
if not os.path.exists("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_members.pickle"):
    used_indexes = 0
    ...
    for iteration in range(amount_iterations):
        ...
        outputs_regressor_member_iteration = outputs_regressor_train[used_indexes:used_indexes+max_data_amount]
        outputs_regressor_non_member_iteration = outputs_regressor_test[used_indexes:used_indexes+max_data_amount]
        ...
        for data_amount in data_amounts:
            ...
            outputs_regressor_non_member = outputs_regressor_non_member_iteration[:data_amount]
            ...
            experiment_1_members_data_amount["outputs_regressor_member"] = outputs_regressor_member
            experiment_1_members_data_amount["outputs_regressor_non_member"] = outputs_regressor_non_member
            experiment_1_members_iteration[f"data amount: {data_amount}"] = experiment_1_members_data_amount
        experiment_1_members[f"iteration: {iteration}"] = experiment_1_members_iteration
        with open("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_members.pickle", "wb") as f:
            pickle.dump(experiment_1_members, f)
else:
    # open pickle
    with open(
            "/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_members.pickle", "rb"
    ) as f:
        experiment_1_members = pickle.load(f)

#experiment 1.1: random data used for each data amount
if not os.path.exists("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_1_members.pickle"):
    ...
    for iteration in range(amount_iterations):
        ...
        for data_amount in data_amounts:
            ...
            experiment_1_members_data_amount["outputs_regressor_non_member"] = outputs_regressor_non_member
            experiment_1_members_iteration[f"data amount: {data_amount}"] = experiment_1_members_data_amount
        experiment_1_1_members[f"iteration: {iteration}"] = experiment_1_members_iteration
        with open("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_1_members.pickle", "wb") as f:
            pickle.dump(experiment_1_1_members, f)
else:
    # open pickle
    with open(
            "/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_1_members.pickle", "rb"
    ) as f:
...
if not os.path.exists("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_2_members.pickle"):
    used_indexes = 0
    max_data_amount = max(data_amounts)
    outputs_regressor_member_total = outputs_regressor_train.shape[0]
    ...
    for iteration in range(amount_iterations):
        ...
        for data_amount in data_amounts:
            ...
            experiment_1_members_data_amount["outputs_regressor_non_member"] = outputs_regressor_non_member
            experiment_1_members_iteration[f"data amount: {data_amount}"] = experiment_1_members_data_amount
        experiment_1_2_members[f"iteration: {iteration}"] = experiment_1_members_iteration
        with open("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_2_members.pickle", "wb") as f:
            pickle.dump(experiment_1_2_members, f)
else:
    # open pickle
    with open(
            "/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_2_members.pickle", "rb"
    ) as f:
        experiment_1_2_members = pickle.load(f)

#experiment 1.3: same members per iteration, always same non-members
if not os.path.exists("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_3_members.pickle"):
    used_indexes = 0
    max_data_amount = max(data_amounts)
    ...
    for iteration in range(amount_iterations):
        ...
        for data_amount in data_amounts:
            ...
            experiment_1_members_data_amount["outputs_regressor_non_member"] = outputs_regressor_non_member
            experiment_1_members_iteration[f"data amount: {data_amount}"] = experiment_1_members_data_amount
        experiment_1_3_members[f"iteration: {iteration}"] = experiment_1_members_iteration
        with open("/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_3_members.pickle", "wb") as f:
            pickle.dump(experiment_1_members, f)
else:
    # open pickle
    with open(
            "/home/inafen/jupyter_notebooks/dataset_and_membership_inference/experiment_1_3_members.pickle", "rb"
    ) as f:
...
def plot_iteration(experiment, title:str)-> None:
    ...
    for iteration in range(amount_iterations):
        p_values_iteration = []
        for data_amount in data_amounts:
            p_values_iteration.append(experiment[f"iteration: {iteration}"][f"data amount: {data_amount}"]["p value"])
        plot_p_value(amounts_data= data_amounts,p_values = p_values_iteration, title = title )
...
#plot experiments
plot_mean_all_iterations(experiment_1_members, title = "Exp. 1: Same members, non-members per iteration")
plot_mean_all_iterations(experiment_1_1_members, title = "Exp. 1.1: Random members, non-members for each data amount")
plot_mean_all_iterations(experiment_1_2_members, title = "Exp. 1.2: Same members per iteration, random non-members for each data amount")
plot_mean_all_iterations(experiment_1_3_members, title = "Exp. 1.3: Same members per iteration, always same non-members")