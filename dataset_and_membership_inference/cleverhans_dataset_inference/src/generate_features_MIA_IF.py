...
import time
from cleverhans_dataset_inference.src.attacks import (
    norms_linf_squeezed,
    norms_l1_squeezed,
    norms_l2_squeezed,
    loss_mingd,
...
def get_dataloaders_MIA(
    ...
    if dataset in ["CIFAR10", "CIFAR100"]:
        ...
        cifar_train = data_source(
            "../../data", train=True, download=True, transform=transform_train
        )
        cifar_test = data_source(
            "../../data", train=False, download=True, transform=transform_test
        )
        train_loader = DataLoader(
            cifar_train, batch_size=batch_size, shuffle=train_shuffle
        )
        test_loader = DataLoader(cifar_test, batch_size=batch_size, shuffle=False)

        if pseudo_labels:
            transform_train = transforms.Compose(
                [transforms.ToTensor(), transforms.Lambda(lambda x: x.float())]
            )
            ...
            cifar_train = PseudoDataset(
                aux_data, aux_targets, transform=transform_train
            )
            train_loader = DataLoader(
                cifar_train, batch_size=batch_size, shuffle=train_shuffle
            )
    ...
    if dataset == "MNIST":
        ...
        train_loader = DataLoader(
            mnist_train, batch_size=batch_size, shuffle=train_shuffle
        )
        ...
        tr_normalize = (
            transforms.Normalize(norm_mean[dataset], norm_std[dataset])
            if normalize
            else transforms.Lambda(lambda x: x)
        )
        transform_train = transforms.Compose(
        ...
        if dataset == "ImageNet":
            ...
            try:
                ...
                if concat:
                    ...
                    d_train = data_source(
                        "../data",
                        split="train" if not pseudo_labels else "extra",
                        download=True,
                        transform=transform_train,
                    )
...
def mingd_MIA(
    ...
    for t in range(num_iter):
        if t > 0:
            ...
            remaining[
                remaining_temp
            ] = new_remaining  # IF: changed to remaining_temp to avoid error
        else:
            preds = model(X + delta)
...
def mingd_unoptimized_MIA(
    ...
    for t in range(num_iter):
        ...
        delta.data = torch.min(
            torch.max(delta.detach(), -X), 1 - X
        )  # clip X+delta[remaining] to [0,1]
        delta.grad.zero_()
...
def get_mingd_vulnerability_MIA(
    loader,
    model,
    num_images=1000,
    batch_size=100,
